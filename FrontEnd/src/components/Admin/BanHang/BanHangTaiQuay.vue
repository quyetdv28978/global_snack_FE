...
const logoImageSrc = ref(`${import.meta.env.VITE_BASE_FRONTEND_ENDPOINT}/src/assets/images/logo.jpg`);
...
const schema = yup.object().shape({
    tienKhachDua: yup.number().required('Không được để trống').typeError('Số lượng tồn phải là một số'),
...
<template>
    <Loading v-model:active="uiBlock" :can-cancel="false" :is-full-page="true" :background-color="bgColor" :opacity="1" :lock-scroll="true">
        <template #default>
        ...
        <table style="margin-top: 1rem; border-collapse: collapse; width: 100%">
            ...
            <tbody>
                <tr v-for="(hdct, itemObjKey) in dsHDCT">
                    <td>{{ itemObjKey + 1 }}</td>
                    <td>{{ hdct.sanPhamChiTiet.sanPham.ten }} - {{ hdct.sanPhamChiTiet.trongLuong.value }}</td>
                    <td>{{ hdct.soLuong }}</td>
        ...
        <div class="row">
            <div class="col-9" style="margin-top: -20px; height: 340px">
                <div class="card gap-3" style="height: 100%">
                    <DataTable :value="dsHDCho" dataKey="id" v-model:selection="selectedHoaDon" tableStyle="height: 200px" selectionMode="single" @rowSelect="tableHoaDonRowSelected" scrollable scrollHeight="205px" showGridlines>
                        <template #header>
                            <div class="flex justify-content-between align-items-center">
                                <h5 class="m-0">Hóa đơn chờ</h5>
                                <Button label="Tạo hoá đơn" icon="pi pi-plus" size="small" outlined :loading="loading" @click="taoHDCho" />
                            </div>
                        </template>
                        <template #empty>
                        ...
                        <Column header="Khách hàng">
                            <template #body="slotProps">
                                <div class="flex justify-content-between align-items-center">
                                    <span :class="[slotProps.data.user === null ? 'text-red-500 font-semibold' : null]"> {{ slotProps.data.user === null ? 'Khong co user' : slotProps.data.user.ten }} </span>
                                    <i class="pi pi-user-edit hover:text-cyan-600" style="font-size: 1.2rem" @click="showKhachHangDialog"></i>
                                    <DynamicDialog />
                                </div>
                            </template>
                        </Column>
                        <Column class="text-center">
                            <template #body="slotProps">
                                <Button @click="confirmHuyHD($event, slotProps.data)" icon="pi pi-trash" severity="danger" outlined rounded></Button>
                            </template>
                        </Column>
                    </DataTable>
            ...
            <div class="Field col-12 md:col-9" style="margin-top: -15px; height: 340px; margin-bottom: 10px">
                <div class="card" style="width: 100%; height: 100%">
                    <DataTable :value="dsHDCT" tableStyle="height: 200px" dataKey="id" scrollable scrollHeight="205px" showGridlines>
                        ...
                        <template #empty>
                            <div class="flex flex-column justify-content-center align-items-center">
                                <LottieAnimation class="w-1" :animation-data="EmptyCartLottie" :loop="true" :auto-play="true" :speed="0.8" />
                                <h6>Không có dữ liệu.</h6>
                            </div>
                        </template>
                        ...
                        <Column field="sanPhamChiTiet.sanPham.ten" header="Tên sản phẩm">
                            <template #body="slotProps">
                                Mũ bảo hiểm {{ slotProps.data.sanPhamChiTiet.sanPham.ten }} -
                        ...
                        <Column header="Số lượng" class="w-2 text-center" :pt="{ headerContent: { class: 'justify-content-center' } }">
                            <template #body="slotProps">
                                <InputNumber
                        ...
                        <Column header="Thành tiền">
                            <template #body="slotProps">
                                {{ formatCurrency((parseInt(slotProps.data.donGia) - parseInt(slotProps.data.chietKhau)) * parseInt(slotProps.data.soLuong)) }}
                            </template>
                        </Column>
                    ...
                    <OverlayPanel ref="op1" appendTo="body" showCloseIcon> </OverlayPanel>
                </div>
            </div>
            ...
            <div class="Field col-12 md:col-12" style="">
                        ...
                        <template #header>
                            <div class="flex justify-content-between align-items-center">
                                ...
                                <span class="p-input-icon-left">
                                    ...
                                    <InputText class="w-20rem" v-model="sanPhamFilter['global'].value" placeholder="Tìm kiếm theo từ khoá..." />
                                </span>
                                <Button size="small" severity="secondary" icon="pi pi-filter-slash" label="Xoá bộ lọc" outlined @click="clearSanPhamFilter" />
                            </div>
                        </template>
                        <Column field="ma" header="Mã SP"></Column>
                        <Column field="sanPham.ten" header="Tên sản phẩm">
                            <template #body="slotProps"> {{ slotProps.data.sanPham.ten }} </template>
                        </Column>

                        <Column header="Trọng lượng" filterField="trongLuong.value" :showFilterMatchModes="false" :filterMenuStyle="{ width: '14rem' }">
                            <template #body="{ data }">
                            ...
                            <template #filterapply="slotProps">
                                <Button size="small" icon="pi pi-check" label="Lọc" severity="success" rounded @click="slotProps.filterCallback()" />
                            </template>
                            <template #filterclear="slotProps">
                                <Button size="small" label="Huỷ" icon="pi pi-times" severity="danger" rounded @click="slotProps.filterCallback()" />
                            </template>
                        </Column>
              ...
              <template #filterclear="slotProps">
                <Button size="small" label="Huỷ" icon="pi pi-times" severity="danger" rounded
                        ...
                        <Column filterField="sanPham.loai.ten" header="Loại" :showFilterMatchModes="false" :filterMenuStyle="{ width: '14rem' }">
                            <template #body="{ data }">
                                <div class="flex align-items-center gap-2">
                        ...
                        <Column filterField="sanPham.thuongHieu.ten" header="Thương hiệu" :showFilterMatchModes="false" :filterMenuStyle="{ width: '14rem' }">
                            <template #body="{ data }">
                                <div class="flex align-items-center gap-2">
                            ...
                            <template #filter="{ filterModel }">
                                <Dropdown v-model="filterModel.value" :options="dsThuongHieu" placeholder="Tất cả" class="p-column-filter">
                                    <template #option="slotProps">
                                        <div class="flex align-items-center gap-2">
                                            <span>{{ slotProps.option }}</span>
                                        </div>
                                    ...
                                    <div style="display: flex">
                                        <div style="margin-top: 20px; margin-right: 10px">
                                            <span class="p-float-label">
                                                <InputNumber id="number-input" v-model="soLuong" :class="{ 'p-invalid': soLuongError }" :min="1" :max="dataOverlay.soLuongTon" />
    ...
    <Dialog v-model:visible="addDialog" :style="{ width: '500px' }" header="Thanh toán" :modal="true">
        ...
        <form @submit="onSubmit">
            <div class="flex align-items-center" style="margin-top: 20px">
                ...
                <div style="margin-left: 60px">
                    <InputNumber v-model="tienKhachDua" inputId="integeronly" @input="tienKhachDuaInputEvent" :class="{ 'p-invalid': tienKhachDuaError }" />
                    <br />
            ...
            <div class="flex">
                <div>
                    <Dropdown
                        ...
                        @change="onPhuongThucThanhToanChange"
                        :class="{ 'p-invalid': PhuongThucThanhToanToanError }"
                    />
                    <div style="width: 200px">
                        <small class="p-error">{{ PhuongThucThanhToanToanError }}</small>
                    </div>
                </div>
                <div style="margin-left: -40px">
                    <Dropdown
                        v-model="hinhThucGiaoHang"
                        :options="APP_CONSTANT.HINH_THUC_GIAO_HANG"
                        optionLabel="ten"
                        placeholder="Hình thức giao hàng"
                        class="w-full md:w-14rem"
                        style="margin-left: 85px"
                        @change="onHinhThucGiaoHangChange"
                        :class="{ 'p-invalid': hinhThucThanhToanError }"
                    />
                    <div style="margin-left: 90px; width: 200px; height: 0px">
                        <small class="p-error">{{ hinhThucThanhToanError }}</small>
                    </div>
                </div>
            </div>
            <div class="card" v-if="diaChiDialog == true" style="overflow-y: scroll; width: 95%; height: 200px">
                <div style="margin-left: 300px; height: 25px; width: 80px; border: 1px solid blue; border-radius: 20px; text-align: center">
            ...
            <Button :disabled="tienKhachDua < thanhTien || check" label="Thanh toán" severity="warning" raised type="submit" style="margin-top: 20px; margin-left: 150px" />
        </form>
    </Dialog>
    <Dialog v-model:visible="addChonHoaDonDialog" :style="{ width: '500px' }" header="Confirm" :modal="true">
        ...
        <Button label="Không" severity="secondary" @click="addChonHoaDonDialog = false" raised style="margin-top: 20px; margin-left: 150px" />
