...
const loadData = async () => {
    ...
    useHD.dataHoanTraHoanTien = useHD.dataHoanTraHoanTien;
};
...
const searchDate = async () => {
    ...
    if (startDates == null || endDates == null) {
        ...
        const respone = await useHD.searchDateByTrangThai(startDates, endDates, 'ngayTao', 7);
        data.value = respone;
    } else {
        const respone = await useHD.searchDateByTrangThai(startDates, endDates, typeSearchDate.value.value, 7);
        data.value = respone;
    }
};
...
<template>
    ...
    <DataTable ref="dt" :value="useHD.dataHoanTraHoanTien" v-model:selection="selectedProducts" dataKey="id"
